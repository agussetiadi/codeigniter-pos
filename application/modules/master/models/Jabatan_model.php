...
class Jabatan_model extends CI_model
{
	...
	public function view_all(){
		...
		$ex =  $this->db->get_where("apps_jabatan", $array);
		$query = $ex->result_array();

		$totalFiltered = $this->db->get_where("apps_jabatan",$array)->num_rows();
		...
		if( !empty($requestData['search']['value']) ) {
		    ...
		    $this->db->limit($requestData['length'],$requestData['start']);
		    $this->db->like("jabatan_name",$src);
		  
		    $d = $this->db->get_where("apps_jabatan",$array);
		...
		foreach ($query as $key => $row) {
	    	...
	    	$nestedData[] = $i.'<input class="jabatan_id" type="hidden" value="'.$row['jabatan_id'].'">';;
	    	$nestedData[] = $row['jabatan_name'].'<input class="jabatan_name" type="hidden" value="'.$row['jabatan_name'].'">';
	    	...
	    	<a href="'.base_url()."master/jabatan/delete_jabatan/".$row['jabatan_id'].'" class="btn-delete"><button class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></button></a>';
