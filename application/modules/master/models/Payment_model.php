...
class Payment_model extends CI_model
{
	...
	public function view_all(){
		...
		$ex =  $this->db->get_where("pos_payment", $array);
		$query = $ex->result_array();

		$totalFiltered = $this->db->get_where("pos_payment",$array)->num_rows();
		...
		if( !empty($requestData['search']['value']) ) {
		    ...
		    $this->db->limit($requestData['length'],$requestData['start']);
		    $this->db->like("payment_name",$src);
		  
		    $d = $this->db->get_where("pos_payment",$array);
		...
		foreach ($query as $key => $row) {
	    	...
	    	$nestedData[] = $i.'<input class="payment_id" type="hidden" value="'.$row['payment_id'].'">';;
	    	$nestedData[] = $row['payment_name'].'<input class="payment_name" type="hidden" value="'.$row['payment_name'].'">';
	    	...
	    	<a href="'.base_url()."master/payment/delete_payment/".$row['payment_id'].'" class="btn-delete"><button class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></button></a>';
