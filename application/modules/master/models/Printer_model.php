...
class Printer_model extends CI_model
{
	...
	public function view_all(){
		...
		$ex =  $this->db->get_where("apps_printer", $array);
		$query = $ex->result_array();

		$totalFiltered = $this->db->get_where("apps_printer",$array)->num_rows();
		...
		if( !empty($requestData['search']['value']) ) {
		    ...
		    $this->db->limit($requestData['length'],$requestData['start']);
		    $this->db->like("printer_name",$src);
		  
		    $d = $this->db->get_where("apps_printer",$array);
		...
		foreach ($query as $key => $row) {
	    	...
	    	$nestedData[] = $i.'<input class="printer_id" type="hidden" value="'.$row['printer_id'].'">';;
	    	$nestedData[] = $row['printer_name'].'<input class="printer_name" type="hidden" value="'.$row['printer_name'].'">';
	    	...
	    	<a href="'.base_url()."master/printer/delete_printer/".$row['printer_id'].'" class="btn-delete"><button class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></button></a>';
