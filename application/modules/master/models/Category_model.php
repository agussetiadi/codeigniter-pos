...
class Category_model extends CI_model
{
	...
	public function view_all(){
		...
		$ex =  $this->db->get_where("pos_category", $array);
		$query = $ex->result_array();

		$totalFiltered = $this->db->get_where("pos_category",$array)->num_rows();
		...
		if( !empty($requestData['search']['value']) ) {
		    ...
		    $d = $this->db->get_where("pos_category",$array);
		...
		foreach ($query as $key => $row) {
			...
			$num_item = $this->db->get_where("pos_item", array('category_id' => $row['category_id']))->num_rows();
	    	...
	    	$nestedData[] = $i.'<input class="category_id" type="hidden" value="'.$row['category_id'].'">';;
	    	$nestedData[] = $row['category_name'].'<input class="category_name" type="hidden" value="'.$row['category_name'].'">';
	    	$nestedData[] = $num_item;
	    	...
	    	<a href="'.base_url()."master/category/delete_category/".$row['category_id'].'" class="btn-delete"><button class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></button></a>';
